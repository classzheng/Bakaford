...
namespace Bakaford {
	...
	class LambdaAbst {  // Constructor of Axioms
		...
		public: LambdaAbst(Element a, Element c) : antecedent(a), consequence(c), lambda() {
			...
			AtomicFormula af(LAMCHAR+"(" + AtomicFormula(antecedent).literal() + ")." + c.var, tmpType);
			lambda = af();
			lambda.et=Lambda;
	...
	namespace Handle {
		...
		std::pair<Element,Reference> induction(Element &cartpair, unsigned markindex) {
			...
			Element g("ind_g"+std::to_string(markindex)+"(" + A.var + "," + B.var + ARWCHAR + "C(-))", &Universe, CustomType);
